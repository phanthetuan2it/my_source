...
function format_giaban($giaban, $dinhdang, $duoi) {
  return $giaban === 0 ? _lienhe : number_format($giaban, 0, 0, $dinhdang).$duoi;
}
...
function phivanchuyen($tinhthanh) {
  ...
  for($i=0;$i<$max;$i++){
    ...
    if($tinhthanh=='24' && $product_phi['phi_hcm'] > $phivanchuyen) {
      $phivanchuyen = $product_phi['phi_hcm'];
    } else if($product_phi['phi_khac'] > $phivanchuyen) {
...
function phanquyen_edit($quyen, $role, $vitri) {
  ...
  if($role == 3) {
    ...
    for($i=0;$i<count($vitri_s);$i++){
        if($vitri_s[$i] == $vitri && in_array($i.'|1',$sua_s) ){
          $kiemtra = 1;
        } 
      }
  }
...
function upload_photos($file, $extension, $folder, $newname = '') {
  if(isset($file) && !$file['error']) {
    ...
    if (!copy($file["tmp_name"], $folder.$file['name']) && !move_uploaded_file($file["tmp_name"], $folder.$file['name'])) {
      return false;
    }
    return $file['name'];
  }
...
function pagination_home($query,$per_page=10,$page=1,$url='?'){   
  ...
  if($lastpage > 1) {
    ...
    // $pagination .= "<li class='page_info'>Trang {$page} of {$lastpage}</li>";
    if ($page > 1) {
      $pagination.= "<li class='page_button'><a href='{$url}&page={$firstpage}' data-page='1'>{$firstlabel}</a></li>";
    }

    if ($page > 1) {
      $pagination.= "<li class='page_button'><a href='{$url}&page={$prev}' data-page='".($page>1?$page-1:1)."'>{$prevlabel}</a></li>";
    }
...
// ACQUIRE THE ARGUMENTS - MAY NEED SOME SANITY TESTS?
  ...
  if($func=='imagejpeg') {
    $func($canvas, $folder.$new_file,75);
  } else {
    $func($canvas, $folder.$new_file,floor ($quality * 0.075));
  }
  return $new_file;
}
...
function pagesListLimitadmin($url , $totalRows , $pageSize = 5, $offset = 5){
  ...
  for($j = $from; $j <= $to; $j++) {
    if ($j == $currentPage) {
        $links = $links . "<li><a href='#' class='active'>{$j}</a></li>";
    } else {
      $qt = $url. "&p={$j}";
...
function format_size ($rawSize) {
  if ($rawSize / 1048576 > 1) {
    ...
    return ($rawSize / 1024 > 1) ? round($rawSize / 1024, 1) . ' KB' : round($rawSize, 1) . ' Bytes';
  }
}
