<?php if(!defined('_source')) die("Error");
		...
		if ($count_km > 0)
		{
			transfer("Email Đã Tồn Tại . vui lòng chọn email khác", "http://".$config_url."/");
		}
		...
		$file_name=images_name($_FILES['file']['name']);
			if($photo = upload_image("file", 'jpg|jpeg|png|JPG|JPEG|PNG', _upload_thanhvien_l,$file_name)){
				$data['photo'] = $photo;	
				$data['thumb'] = create_thumb($data['photo'], 100, 100, _upload_thanhvien_l,$file_name.'avatar',1);	
			}
		...
		$mail->CharSet = "utf-8";	
			...
			$body .= '
				...
				<tr>
					<td colspan="2"><a href="http://'.$config_url.'/activated/'.$mathanhvien.'.htm">http://'.$config_url.'/activated/'.$mathanhvien.'.htm</a></td>
				</tr>
		...
		if($d->insert($data))
		{
			...
			{
				transfer("Đăng Ký Không Thành Công .Vui lòng đăng ký lại .", "http://".$config_url."/");
			}
