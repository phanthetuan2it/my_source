...
switch($act){
	...
	case "man_color":
		get_colors();
		$template = "product/color/items";
		break;
	case "add_color":		
		$template = "product/color/item_add";
		break;
	case "edit_color":		
		get_color();
		$template = "product/color/item_add";
...
function get_colors(){
	...
	if($_REQUEST['id_p']!='')
	{
		$where.=" and id_p=".$_REQUEST['id_p'];
		$link_add .= "&id_p=".$_GET['id_p'];
	}
...
function get_color(){
	...
	$sql = "select * from #_product_photo_color where id_color='".$id."' and type='".$_GET['type']."' order by stt,id desc ";
	$d->query($sql);
	$ds_photo = $d->result_array();	
}
...
function save_color(){
	...
	if($id){
		...
		if($d->update($data)){
			...
			if (isset($_FILES['files'])) {
	            for($i=0;$i<count($_FILES['files']['name']);$i++){
	            	if($_FILES['files']['name'][$i]!=''){
						...
						$data1['id_color'] = $id;
						$data1['hienthi'] = 1;
						$d->setTable('product_photo_color');
						$d->insert($data1);
			...
			redirect('http://'.$config_url.'/admin/index.php?com='.$_GET['com'].'&act=man_color&id_p='.$_GET['id_p'].'&type='.$_GET['type']);
		}
		else
			transfer("Cập nhật dữ liệu bị lỗi", $_SESSION['links_re']);
	}else{
...
function delete_color(){
	...
	if(isset($_GET['id'])){
		...
		$d->reset();
		$sql = "select id,photo,thumb from #_product_photo_color where id_color='".$id."'";
		$d->query($sql);
		$photo_lq = $d->result_array();
		if(count($photo_lq)>0){
			for($i=0;$i<count($photo_lq);$i++){
		...
		$sql = "delete from #_product_photo_color where id_color='".$id."'";
		$d->query($sql);
