...
function get_items(){
	...
	$url_link='index.php?com=yahoo&act=man&type='.$_REQUEST['type'];		
}
...
function get_item(){
	...
	if(!$id)
		transfer("Không nhận được dữ liệu", "index.php?com=yahoo&act=man&type=".$_REQUEST['type']);
	
	$sql = "select * from #_yahoo where id='".$id."'";
	$d->query($sql);
	if($d->num_rows()==0) transfer("Dữ liệu không có thực", "index.php?com=yahoo&act=man&type=".$_REQUEST['type']);
	$item = $d->fetch_array();
}
...
function save_item(){
	...
	if($id){ // cap nhat
		...
		$d->setWhere('id', $id);
		if($d->update($data))
			header("Location:index.php?com=yahoo&act=man&type=".$_REQUEST['type']);
		else
			transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=yahoo&act=man&type=".$_REQUEST['type']);
	}else{ // them moi
		if($photo = upload_image("file", 'jpg|png|gif|JPG|jpeg|JPEG', _upload_hinhanh,$file_name)){
			$data['photo'] = $photo;
		...
		if($d->insert($data))
			...
			transfer("Lưu dữ liệu bị lỗi", "index.php?com=yahoo&act=man&type=".$_REQUEST['type']);
	}
}
...
function delete_item(){
	...
	if(isset($_GET['id'])){
		...
		if($d->query($sql))
			...
			transfer("Xóa dữ liệu bị lỗi", "index.php?com=yahoo&act=man&type=".$_REQUEST['type']);
	}else transfer("Không nhận được dữ liệu", "index.php?com=yahoo&act=man&type=".$_REQUEST['type']);
}
#--------------------------------------------------------------------------------------------- photo
?>