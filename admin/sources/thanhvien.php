...
function save_item(){
	...
	if($id){ // cap nhat
		...
		if($photo = upload_image("file", 'jpg|png|gif|JPG|jpeg|JPEG', _upload_thanhvien,$file_name)){
			$data['photo'] = $photo;
			$data['thumb'] = create_thumb($data['photo'], 200, 200, _upload_thanhvien,$file_name,1);	
			$d->setTable('thanhvien');
			$d->setWhere('id', $id);
			$d->select();
			if($d->num_rows()>0){
				$row = $d->fetch_array();
				delete_file(_upload_thanhvien.$row['photo']);	
				delete_file(_upload_thanhvien.$row['thumb']);				
			}
		}
