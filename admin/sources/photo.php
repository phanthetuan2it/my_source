...
function save_photo(){
	...
	if($id){
		...
		for($i=0; $i<1; $i++){		
			$upload_image=false;
			foreach ($config['lang'] as $key => $value) {
				if($data['photo_'.$key] = upload_image("file_".$key.$i, 'Jpg|jpg|png|gif|JPG|jpeg|JPEG', _upload_hinhanh,$file_name.$key.$i))
				{		
					$data['thumb_'.$key] = create_thumb($data['photo_'.$key], _width_thumb, _height_thumb , _upload_hinhanh,$file_name.$key.$i.'thumb',_style_thumb);
					$upload_image=true;	
					$data['ten_'.$key] = $_POST['ten_'.$key.$i];	
				}#if for
			}#foreach
			if($upload_image){
				$data['stt'] = $_POST['stt'.$i];
				$data['id_list'] = (int)$_POST['id_list'.$i];
